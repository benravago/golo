...
import static golo.lang.Messages.prefixed;
...
public class LocalReferenceAssignmentAndVerificationVisitor extends AbstractGoloIrVisitor {
  ...
  public void visitAssignmentStatement(AssignmentStatement assignmentStatement) {
    ...
    if (redeclaringReferenceInBlock(assignmentStatement, reference, assignedReferences)) {
      ...
      errorMessage(ASSIGN_CONSTANT, assignmentStatement, message("assign_constant", reference.getName()));
    }
  ...
  private boolean redeclaringReferenceInBlock(AssignmentStatement assignmentStatement, LocalReference reference,
      Set<LocalReference> assignedReferences) {
    return !reference.isSynthetic() && assignmentStatement.isDeclaring()
        && referenceNameExists(reference, assignedReferences);
  }

  private boolean assigningConstant(LocalReference reference, Set<LocalReference> assignedReferences) {
    return reference.isConstant() && (assignedReferences.contains(reference)
  ...
  public void visitReferenceLookup(ReferenceLookup referenceLookup) {
    ...
    if (!table.hasReferenceFor(referenceLookup.getName())) {
      errorMessage(UNDECLARED_REFERENCE, referenceLookup, message("undeclared_reference", referenceLookup.getName(),
          !functionStack.isEmpty() ? message("in_function", functionStack.peek().getName()) : ""));
    }
  ...
  public void visitLoopBreakFlowStatement(LoopBreakFlowStatement loopBreakFlowStatement) {
    if (loopBreakFlowStatement.getEnclosingLoop() == null) {
      errorMessage(BREAK_OR_CONTINUE_OUTSIDE_LOOP, loopBreakFlowStatement, message("break_or_continue_outside_loop"));
    }
  }
