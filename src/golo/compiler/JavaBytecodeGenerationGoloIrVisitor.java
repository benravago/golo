...
final class JavaBytecodeGenerationGoloIrVisitor implements GoloIrVisitor {
  ...
  private static final Handle FUNCTION_INVOCATION_HANDLE = makeHandle("FunctionCallSupport", "[Ljava/lang/Object;");
  private static final Handle OPERATOR_HANDLE = makeHandle("OperatorSupport", "I");
  private static final Handle METHOD_INVOCATION_HANDLE = makeHandle("MethodInvocationSupport", "[Ljava/lang/Object;");
  ...
  private static final boolean USE_TCE = golo.lang.Runtime.loadBoolean("golo.optimize.tce", "GOLO_OPTIMIZE_TCE", true);
  ...
  private static final class Context {
    ...
    void enterFunction(GoloFunction function, ClassWriter classWriter) {
      this.currentMethodVisitor = classWriter.visitMethod(functionFlags(function), function.getName(),
          function.getMethodType().toMethodDescriptorString(), null, null);
  ...
  private static Handle makeHandle(String methodName, String description) {
    return new Handle(H_INVOKESTATIC, "org/eclipse/golo/runtime/" + methodName, "bootstrap",
        "(Ljava/lang/invoke/MethodHandles$Lookup;Ljava/lang/String;Ljava/lang/invoke/MethodType;" + description
  ...
  public List<CodeGenerationResult> generateBytecode(GoloModule module) {
    ...
    this.generationResults
        .add(new CodeGenerationResult(classWriter.toByteArray(), module.getPackageAndClass(), module.sourceFile()));
  ...
  public void visitModule(GoloModule module) {
    ...
    classWriter.visit(V1_8, ACC_PUBLIC | ACC_SUPER | deprecatedFlag(module), module.getPackageAndClass().toJVMType(),
        null, JOBJECT, null);
  ...
  public void visitLocalReference(LocalReference moduleState) {
    if (moduleState.isModuleState()) {
      ...
      classWriter.visitField(ACC_PRIVATE | ACC_STATIC | deprecatedFlag(moduleState), name, TOBJECT, null, null)
          .visitEnd();

      MethodVisitor mv = classWriter.visitMethod(ACC_PRIVATE | ACC_STATIC | ACC_SYNTHETIC, name, "()Ljava/lang/Object;",
          null, null);
      mv.visitCode();
  ...
  private void writeAugmentationApplicationsMetaData(GoloModule module) {
    ...
    writeMetaData("augmentationApplications",
        applications.stream().map(Augmentation::getTarget).map(PackageAndClass::toString).toArray(String[]::new));
    ...
    MethodVisitor mv = classWriter.visitMethod(ACC_PUBLIC | ACC_STATIC | ACC_SYNTHETIC, "$augmentationApplications",
  ...
  private void writeImportMetaData(Set<ModuleImport> imports) {
    writeMetaData("imports",
        imports.stream().map(ModuleImport::getPackageAndClass).map(PackageAndClass::toString).toArray(String[]::new));
  }

  private void writeAugmentsMetaData(GoloModule module) {
    writeMetaData("augmentations", module.getAugmentations().stream().map(Augmentation::getTarget)
  ...
  public void visitAugmentation(Augmentation augmentation) {
    generateAugmentationBytecode(augmentation.enclosingModule(), augmentation.getTarget(), augmentation.getFunctions(),
        augmentation.metadata("annotations"));
  }

  @Override
  public void visitNamedAugmentation(NamedAugmentation namedAugmentation) {
    generateAugmentationBytecode(namedAugmentation.enclosingModule(), namedAugmentation.getPackageAndClass(),
        namedAugmentation.getFunctions(), namedAugmentation.metadata("annotations"));
  }

  private void generateAugmentationBytecode(GoloModule parentModule, PackageAndClass target,
      Collection<GoloFunction> functions, Object annotations) {
    ...
    mainClassWriter.visitInnerClass(augmentationClassInternalName, outerName, mangledClass, ACC_PUBLIC | ACC_STATIC);
  ...
  private void addFunctionAnnotations(GoloFunction function) {
    ...
    if (function.isDecorated()) {
      AnnotationVisitor annotation = context.currentMethodVisitor.visitAnnotation("Lgololang/annotations/DecoratedBy;",
          true);
    ...
    if (function.isMacro()) {
      ...
      if (function.isContextualMacro()) {
        context.currentMethodVisitor.visitAnnotation("Lorg/eclipse/golo/compiler/macro/ContextualMacro;", true)
            .visitEnd();
      }
  ...
  public void visitBlock(Block block) {
    ...
    for (LocalReference localReference : block.getReferenceTable().ownedReferences()) {
      ...
      context.currentMethodVisitor.visitLocalVariable(localReference.getName(), TOBJECT, null, blockStart, blockEnd,
  ...
  public void visitConstantStatement(ConstantStatement constantStatement) {
    ...
    if (value == null) {
      ...
      context.currentMethodVisitor.visitMethodInsn(INVOKESTATIC, "java/lang/Integer", "valueOf",
          "(I)Ljava/lang/Integer;", false);
    } else if (value instanceof Long) {
      long l = (Long) value;
      loadLong(context.currentMethodVisitor, l);
      context.currentMethodVisitor.visitMethodInsn(INVOKESTATIC, "java/lang/Long", "valueOf", "(J)Ljava/lang/Long;",
          false);
    } else if (value instanceof Boolean) {
      boolean b = (Boolean) value;
      loadInteger(context.currentMethodVisitor, b ? 1 : 0);
      context.currentMethodVisitor.visitMethodInsn(INVOKESTATIC, "java/lang/Boolean", "valueOf",
      ...
      context.currentMethodVisitor.visitMethodInsn(INVOKESPECIAL, "java/math/BigDecimal", "<init>",
          "(Ljava/lang/String;)V", false);
    } else if (value instanceof BigInteger) {
      context.currentMethodVisitor.visitTypeInsn(NEW, "java/math/BigInteger");
      context.currentMethodVisitor.visitInsn(DUP);
      context.currentMethodVisitor.visitLdcInsn(value.toString());
      context.currentMethodVisitor.visitMethodInsn(INVOKESPECIAL, "java/math/BigInteger", "<init>",
          "(Ljava/lang/String;)V", false);
    } else if (value instanceof String) {
      context.currentMethodVisitor.visitLdcInsn(value);
    } else if (value instanceof Character) {
      loadInteger(context.currentMethodVisitor, (Character) value);
      context.currentMethodVisitor.visitMethodInsn(INVOKESTATIC, "java/lang/Character", "valueOf",
          "(C)Ljava/lang/Character;", false);
    } else if (value instanceof ClassReference) {
      context.currentMethodVisitor.visitInvokeDynamicInsn(((ClassReference) value).toJVMType(), "()Ljava/lang/Class;",
          CLASSREF_HANDLE);
    } else if (value instanceof Double) {
      double d = (Double) value;
      context.currentMethodVisitor.visitLdcInsn(d);
      context.currentMethodVisitor.visitMethodInsn(INVOKESTATIC, "java/lang/Double", "valueOf", "(D)Ljava/lang/Double;",
          false);
    } else if (value instanceof Float) {
      float f = (Float) value;
      context.currentMethodVisitor.visitLdcInsn(f);
      context.currentMethodVisitor.visitMethodInsn(INVOKESTATIC, "java/lang/Float", "valueOf", "(F)Ljava/lang/Float;",
          false);
    } else {
  ...
  public void visitMethodInvocation(MethodInvocation methodInvocation) {
    ...
    bootstrapArgs.addAll(argumentNames);
    context.currentMethodVisitor.visitInvokeDynamicInsn(methodInvocation.getName().replaceAll("\\.", "#"),
        goloFunctionSignature(methodInvocation.getArity() + 1), METHOD_INVOCATION_HANDLE, bootstrapArgs.toArray());
  }
  ...
  public void visitAssignmentStatement(AssignmentStatement assignmentStatement) {
    ...
    if (reference.isModuleState()) {
      context.currentMethodVisitor.visitInvokeDynamicInsn((klass + "." + reference.getName()).replaceAll("\\.", "#"),
  ...
  public void visitClosureReference(ClosureReference closureReference) {
    ...
    context.currentMethodVisitor.visitInvokeDynamicInsn(target.getName(),
        methodType(FunctionReference.class).toMethodDescriptorString(), CLOSUREREF_HANDLE, klass, (Integer) arity,
        (Boolean) isVarArgs);
    if (syntheticCount > 0) {
      ...
      context.currentMethodVisitor.visitMethodInsn(INVOKEVIRTUAL, "gololang/FunctionReference", "insertArguments",
          "(I[Ljava/lang/Object;)Lgololang/FunctionReference;", false);
      if (isVarArgs) {
        context.currentMethodVisitor.visitLdcInsn(Type.getType(Object[].class));
        context.currentMethodVisitor.visitMethodInsn(INVOKEVIRTUAL, "gololang/FunctionReference", "asVarargsCollector",
