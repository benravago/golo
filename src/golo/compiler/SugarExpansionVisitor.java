...
import static golo.lang.ir.ConditionalBranching.branch;
import static golo.lang.ir.MethodInvocation.invoke;
...
public class SugarExpansionVisitor extends AbstractGoloIrVisitor {
  ...
  private boolean useNewStyleDestruct() {
    ...
    return golo.lang.Runtime.loadBoolean("true", "golo.destruct.newstyle", "GOLO_DESTRUCT_NEWSTYLE", true);
  }
  ...
  private ConditionalBranching convertCaseToConditional(CaseStatement caseStatement) {
    ...
    ConditionalBranching branch = branch().condition(lastClause.condition()).whenTrue(lastClause.action())
        .whenFalse(caseStatement.getOtherwise()).positionInSourceCode(lastClause.positionInSourceCode());
    while (!clauses.isEmpty()) {
      lastClause = clauses.removeLast();
      branch = branch().condition(lastClause.condition()).whenTrue(lastClause.action()).elseBranch(branch)
  ...
  public void visitMatchExpression(MatchExpression matchExpression) {
    LocalReference tempVar = LocalReference.of(symbols.next("match")).variable().synthetic();
  ...
  private static Block convertMatchToBlock(MatchExpression matchExpression,
      Function<ExpressionStatement<?>, ? extends GoloStatement<?>> mapping) {
  ...
  private static CaseStatement convertMatchToMappedCase(MatchExpression matchExpression,
      Function<ExpressionStatement<?>, ? extends GoloStatement<?>> mapping) {
  ...
  private FunctionInvocation literalFunctionRefToCall(FunctionRef ref) {
    return FunctionInvocation.of("gololang.Predefined.fun").constant().withArgs(ConstantStatement.of(ref.name()),
        ConstantStatement.of(ClassReference.of(ref.module() == null ? this.module.getPackageAndClass() : ref.module())),
        ConstantStatement.of(ref.arity()), ConstantStatement.of(ref.varargs()));
  ...
  public void visitCollectionComprehension(CollectionComprehension collection) {
    ...
    LocalReference tempVar = LocalReference.of(symbols.next("comprehension")).variable().synthetic();
    ...
    innerBlock.add(invoke("add").withArgs(collection.expression()).on(tempVar.lookup()));

    if (collection.getType() == CollectionLiteral.Type.array || collection.getType() == CollectionLiteral.Type.tuple) {
      mainBlock.add(AssignmentStatement.create(tempVar, invoke("toArray").on(tempVar.lookup()), false));
    }

    if (collection.getType() == CollectionLiteral.Type.tuple) {
      mainBlock.add(AssignmentStatement.create(tempVar,
          FunctionInvocation.of("Tuple.fromArray").withArgs(tempVar.lookup()), false));
    }
  ...
  public void visitForEachLoopStatement(ForEachLoopStatement foreachStatement) {
    LocalReference iterVar = LocalReference.of(symbols.next("forEachIterator")).variable().synthetic();

    // deal with when clause
    Block loopInnerBlock;
    if (foreachStatement.hasWhenClause()) {
      loopInnerBlock = Block
          .of(branch().condition(foreachStatement.getWhenClause()).whenTrue(foreachStatement.getBlock()))
    ...
    if (foreachStatement.isDestructuring()) {
      loopInnerBlock.prepend(DestructuringAssignment.destruct(invoke("next").on(iterVar.lookup())).declaring()
          .varargs(foreachStatement.isVarargs()).to((Object[]) foreachStatement.getReferences()));
    } else {
    ...
    LoopStatement newLoop = LoopStatement.loop()
        .init(AssignmentStatement.create(iterVar, invoke("iterator").on(foreachStatement.getIterable()), true))
        .condition(invoke("hasNext").on(iterVar.lookup())).block(loopInnerBlock);
  ...
  private Block oldDestructuring(DestructuringAssignment assignment) {
    ...
    for (LocalReference ref : assignment.getReferences()) {
      block.add(AssignmentStatement.create(ref, invoke(assignment.isVarargs() && idx == last ? "subTuple" : "get")
          .withArgs(ConstantStatement.of(idx)).on(tmpRef.lookup()), assignment.isDeclaring()));
      idx++;
    }
    return block;
  }
  ...
  private Block newDestructuring(DestructuringAssignment assignment) {
    ...
    for (LocalReference ref : assignment.getReferences()) {
      if ("_".equals(ref.getName())) {
        ...
        block.add(AssignmentStatement.create(ref, invoke("get").withArgs(ConstantStatement.of(idx)).on(tmpRef.lookup()),
            assignment.isDeclaring()));
      }
      idx++;
    }
    block.prepend(AssignmentStatement.create(tmpRef,
        invoke("__$$_destruct").withArgs(ConstantStatement.of(assignment.getReferencesCount()),
            ConstantStatement.of(assignment.isVarargs()),
            CollectionLiteral.create(CollectionLiteral.Type.array, toSkip)).on(assignment.expression()),
        true));
    return block;
  }
