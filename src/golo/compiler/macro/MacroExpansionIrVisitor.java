...
public final class MacroExpansionIrVisitor extends AbstractGoloIrVisitor {
  ...
  public MacroExpansionIrVisitor(ClassLoader loader, boolean defaultRecurse,
  ...
  private MacroInvocation decoratorToMacroInvocation(Decorator decorator, GoloElement<?> function) {
    ...
    if (expr instanceof FunctionInvocation) {
      ...
      return MacroInvocation.call(invocation.getName()).withArgs(invocation.getArguments().toArray())
          .withArgs(function);
    } else if (expr instanceof ReferenceLookup) {
      return MacroInvocation.call(((ReferenceLookup) expr).getName()).withArgs(function);
  ...
  private GoloElement<?> expandMacro(MacroInvocation invocation) {
    debug("try to expand %s", invocation);
    return findMacro(invocation).map(invokeMacroWith(invocation)).orElse(noMacroResult(invocation.getName()));
  }
  ...
  private void expansionFailed(AbstractInvocation<?> invocation, Throwable t) {
    String errorMessage = message("macro_expansion_failed", invocation.getName()) + ' ' + position(invocation) + ".";
