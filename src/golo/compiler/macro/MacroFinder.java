...
import golo.runtime.TypeMatching;
...
class MacroFinder {
  ...
  private static final List<String> DEFAULT_MACROS = unmodifiableList(asList("gololang.macros"));
  ...
  private Optional<MacroFinderResult> findMacro(AbstractInvocation<?> invocation) {
    return getLookupClassNames(invocation).map(loader).filter(java.util.Objects::nonNull).flatMap(Extractors::getMacros)
        .filter(isCandidateMacro(invocation)).map(this::toResult).filter(java.util.Objects::nonNull).findFirst();
  }
  ...
  private Predicate<Method> isCandidateMacro(AbstractInvocation<?> invocation) {
    return method -> method.getName().equals(invocation.getFunctionName())
        ...
        && (TypeMatching.argumentsNumberMatches(method,
            invocation.getArity() + (method.isAnnotationPresent(ContextualMacro.class) ? 1 : 0)
  ...
  private MacroFinderResult toResult(Method method) {
    ...
    return new MacroFinderResult(target, method.isAnnotationPresent(SpecialMacro.class),
