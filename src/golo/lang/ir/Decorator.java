...
public final class Decorator extends GoloElement<Decorator> {
  ...
  private boolean isValidDecoratorExpressoin(ExpressionStatement<?> expr) {
    return expr instanceof ReferenceLookup || expr instanceof FunctionInvocation || expr instanceof ClosureReference
        || (expr instanceof BinaryOperation && OperatorType.ANON_CALL.equals(((BinaryOperation) expr).getType()));
  }

  private void setExpressionStatement(ExpressionStatement<?> expr) {
    if (!isValidDecoratorExpressoin(expr)) {
      throw new IllegalArgumentException(
          "Decorator expression must be a reference or an invocation, got a " + expr.getClass().getSimpleName());
    }
