...
public abstract class TypeWithMembers<T extends TypeWithMembers<T>> extends GoloType<T> {
  ...
  public List<Member> getPublicMembers() {
    return members.stream().filter(Member::isPublic).collect(toList());
  }

  protected Object[] getFullArgs() {
    return members.stream().map(Member::getName).map(ReferenceLookup::of).toArray();
  }
  ...
  protected GoloFunction createFullArgsConstructor() {
    return GoloFunction.function(getName()).synthetic().withParameters(getMemberNames())
