...
public final class GoloModule extends GoloElement<GoloModule> implements FunctionContainer {
  ...
  private static final ModuleImport[] DEFAULT_IMPORTS = { ModuleImport.implicit("gololang.Predefined"),
      ModuleImport.implicit("gololang.StandardAugmentations"), ModuleImport.implicit("gololang"),
    ...
    for (MacroInvocation m : topLevelMacroInvocations) {
      if ("use".equals(m.getName())) {
        ...
        if (name instanceof String) {
          mods.add((String) name);
        } else if (name instanceof ClassReference) {
          mods.add(((ClassReference) name).getName());
        }
      }
    }
    return mods;
  ...
  public boolean isEmpty() {
    return imports.isEmpty() && types.isEmpty() && augmentations.isEmpty() && namedAugmentations.isEmpty()
        && moduleState.isEmpty() && functions.isEmpty() && topLevelMacroInvocations.isEmpty();
  }
