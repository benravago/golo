...
public final class TryCatchFinally extends GoloStatement<TryCatchFinally> {
   ...
   */
  public static TryCatchFinally create(String exceptionName, GoloElement<?> tryBlock, GoloElement<?> catchBlock,
      GoloElement<?> finallyBlock) {
    return new TryCatchFinally().trying(tryBlock).catching(exceptionName, catchBlock).finalizing(finallyBlock);
  }
  ...
  public boolean mustJumpToFinally() {
    return (this.tryBlock.hasReturn() || (this.catchBlock != null && this.catchBlock.hasReturn()))
  ...
  public boolean hasReturn() {
    return (this.tryBlock.hasReturn() && (this.catchBlock == null || this.catchBlock.hasReturn()))
        || (this.finallyBlock != null && this.finallyBlock.hasReturn());
  }
