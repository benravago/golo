...
public final class ReferenceTable {
  ...
  private boolean isLinkedTo(ReferenceTable other) {
    return this == other || this.parent == other || (this.parent != null && this.parent.isLinkedTo(other));
  }
  ...
  public ReferenceTable flatDeepCopy(boolean turnIntoConstants) {
    ...
    for (LocalReference reference : references()) {
      ...
      if (reference.isModuleState()) {
        referenceTable.add(LocalReference.of(refName).kind(reference.getKind()));
        continue;
      }
      if (turnIntoConstants && !table.containsKey(refName)) {
        referenceTable.add(LocalReference.of(refName).synthetic(reference.isSynthetic()));
      } else {
        referenceTable.add(LocalReference.of(refName).synthetic(reference.isSynthetic()).kind(reference.getKind()));
      }
    }
