...
public final class GoloFunction extends ExpressionStatement<GoloFunction>
    implements BlockContainer<GoloFunction>, ToplevelGoloElement, NamedElement {
  ...
  public GoloFunction createDecorator() {
    ...
    return function(this.decoratorRef).decorator().inScope(this.scope).withParameters("__$$_original").returns(expr);
  }
  ...
  public GoloFunction sameSignature() {
    return function(name).local(isLocal).inScope(scope).withParameters(parameterNames).varargs(varargs).synthetic(true);
  }
  ...
  public String toString() {
    return String.format("Function{name=%s, arity=%d, vararg=%s, synthetic=%s, self=%s, macro=%s}", getName(),
        getArity(), isVarargs(), synthetic, syntheticSelfName, isMacro());
  }
  ...
  public MethodType getMethodType() {
    ...
    if (this.isMacro()) {
      ...
      if (this.isContextualMacro()) {
        signature = signature.appendParameterTypes(golo.lang.ir.AbstractInvocation.class);
        i++;
      }
      if (this.isSpecialMacro()) {
        signature = signature.appendParameterTypes(golo.compiler.macro.MacroExpansionIrVisitor.class);
        i++;
      }
      while (i < arity) {
