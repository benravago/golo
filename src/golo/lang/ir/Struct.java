...
public final class Struct extends TypeWithMembers<Struct> implements ToplevelGoloElement {
  ...
  private GoloFunction createFullArgsImmutableConstructor() {
    return GoloFunction.function(getImmutableName()).synthetic().withParameters(getMemberNames()).returns(
        FunctionInvocation.of(getFactoryDelegateName() + "." + IMMUTABLE_FACTORY_METHOD).withArgs(getFullArgs()));
  }
