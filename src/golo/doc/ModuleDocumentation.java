...
public class ModuleDocumentation implements DocumentationElement {
  ...
  public boolean isEmpty() {
    return moduleStates.isEmpty() && functions.isEmpty() && macros.isEmpty() && augmentations.isEmpty()
        && structs.isEmpty() && unions.isEmpty() && namedAugmentations.isEmpty();
  }

  public String goloVersion() {
    return golo.Metadata.VERSION;
  }
  ...
  private class ModuleVisitor implements GoloIrVisitor {
    ...
    public void visitStruct(Struct struct) {
      StructDocumentation doc = new StructDocumentation().parent(parents.peek()).name(struct.getName())
          .documentation(struct.documentation()).line(struct.positionInSourceCode().getStartLine());
      structs.add(doc);
    ...
    public void visitUnion(Union union) {
      this.currentUnion = new UnionDocumentation().parent(parents.peek()).name(union.getName())
          .documentation(union.documentation()).line(union.positionInSourceCode().getStartLine());
    ...
    public void visitUnionValue(UnionValue value) {
      UnionDocumentation.UnionValueDocumentation doc = this.currentUnion.addValue(value.getName())
          .parent(parents.peek()).documentation(value.documentation())
    ...
    public void visitAugmentation(Augmentation augment) {
      ...
      if (!augmentations.containsKey(target)) {
        augmentations.put(target, new AugmentationDocumentation().target(target).parent(parents.peek())
            .augmentationNames(augment.getNames()).line(augment.positionInSourceCode().getStartLine()));
      }
    ...
    public void visitNamedAugmentation(NamedAugmentation augment) {
      NamedAugmentationDocumentation augmentDoc = new NamedAugmentationDocumentation().parent(parents.peek())
          .name(augment.getName()).documentation(augment.documentation())
    ...
    public void visitFunction(GoloFunction function) {
      if (!GoloModule.MODULE_INITIALIZER_FUNCTION.equals(function.getName())) {
        FunctionDocumentation fdoc = new FunctionDocumentation(function.isMacro()).parent(parents.peek())
            .name(function.getName()).documentation(function.documentation()).augmentation(function.isInAugment())
            .line(function.positionInSourceCode().getStartLine()).local(function.isLocal())
            .arguments(function.getParameterNames()).varargs(function.isVarargs());
    ...
    public void visitMember(Member member) {
      currentMemberHolder.addMember(member.getName()).parent(parents.peek()).documentation(member.documentation())
