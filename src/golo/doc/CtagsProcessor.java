...
public class CtagsProcessor extends AbstractProcessor {
  ...
  private void ctagsModule(ModuleDocumentation module) {
    ctagsLine(module.moduleName(), "/^module[:blank:]+" + module.moduleName().replace(".", "\\.") + "$/",
  ...
  private void ctagsAugment(String name, int line) {
    ctagsLine(name, String.format("/^augment[:blank:]+%s/", name.replace(".", "\\.")),
        String.format("a\tline:%s", line));
  }

  private void ctagsAugmentation(String name, int line) {
    ctagsLine(name, String.format("/^augmentation[:blank:]+%s[:blank:]+=[:blank:]+{/", name),
        String.format("na\tline:%s", line));
  }

  private void ctagsStruct(String name, int line) {
    ctagsLine(name, String.format("/^struct[:blank:]+%s[:blank:]+=/", name), String.format("s\tline:%s", line));
  }

  private void ctagsUnion(UnionDocumentation unionDoc) {
    ctagsLine(unionDoc.name(), String.format("/^union[:blank:]+%s[:blank:]+=[:blank:]+{/", unionDoc.name()),
  ...
  private void ctagsUnionValue(String unionName, UnionDocumentation.UnionValueDocumentation valueDoc) {
    ...
    for (MemberDocumentation member : valueDoc.members()) {
      ctagsLine(member.name(), String.format("/[:blank:]+%s[:blank:]+=/", valueDoc.name()),
          String.format("m\tline:%s\taccess:public\tvalue:%s", member.line(), valueDoc.name()));
    }
  }

  private void ctagsImport(String name, int line) {
    ctagsLine(name, String.format("/^import[:blank:]+%s/", name.replace(".", "\\.")),
        String.format("i\tline:%s", line));
  }

  private void ctagsModState(String name, int line) {
    ctagsLine(name, String.format("(let|var)[:blank:]+%s[:blank:]+=/", name),
        String.format("v\taccess:private\tfile:\tline:%s", line));
  }

  private void ctagsStructMember(String struct, String member, int line) {
    ctagsLine(member, String.format("/struct[:blank:]+%s[:blank:]+=/", struct), String
        .format("m\tline:%s\taccess:%s\tstruct:%s", line, (member.charAt(0) == '_') ? "private" : "public", struct));
  }
