...
import static golo.runtime.TypeMatching.argumentsNumberMatches;
import static golo.runtime.TypeMatching.compareTypes;
...
public final class Extractors {
  ...
  public static Stream<Method> getMethods(Class<?> klass) {
    ...
    return Stream.concat(Stream.of(klass.getDeclaredMethods()), Stream.of(klass.getMethods())).distinct()
        .sorted((m1, m2) -> {
  ...
  public static <T extends AnnotatedElement & Member> T checkDeprecation(Class<?> caller, T object) {
    if (object.isAnnotationPresent(Deprecated.class)) {
      Warnings.deprecatedElement((object instanceof Executable ? ((Executable) object).toGenericString()
          : object instanceof Field ? ((Field) object).toGenericString() : object.getName()), caller.getName());
    }
    return object;
  }
