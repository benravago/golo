...
import golo.runtime.augmentation.DefiningModule;
...
class AugmentationMethodFinder extends MethodFinder {
  ...
  private Stream<DefiningModule> getImportedModules(Class<?> sourceClass) {
    return Extractors.getImportedNames(sourceClass).filter(AugmentationMethodFinder::candidateImport).map(loader)
        .filter(Objects::nonNull).map(DefiningModule::ofImport);
  }
  ...
  private Stream<DefiningModule> getCallStack() {
    return Stream.of(Thread.currentThread().getStackTrace()).map(StackTraceElement::getClassName)
        .filter(AugmentationMethodFinder::isCandidateInStackTrace).skip(1).map(loader).filter(Objects::nonNull)
  ...
  private Stream<DefiningModule> getDefiningModules() {
    return Stream
        .of(getLocalCaller(callerClass), getImportedModules(callerClass), getCallStack(),
  ...
  public MethodHandle reorderArguments(Method method, MethodHandle handle) {
    return NamedArgumentsHelper.reorderArguments(method.getName(), NamedArgumentsHelper.getParameterNames(method),
        handle, invocation.argumentNames(), 1, 0);
  }

  @Override
  public MethodHandle find() {
    return getDefiningModules().flatMap(dm -> dm.augmentationsFor(loader, invocation.receiverClass()))
        .flatMap(aug -> aug.methodsMaching(invocation)).min(Comparator.naturalOrder())
        .flatMap(am -> toMethodHandle(am.method())).orElse(null);
  }
}
