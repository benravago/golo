...
import static golo.runtime.Extractors.checkDeprecation;
import static golo.runtime.NamedArgumentsHelper.*;
...
public final class FunctionCallSupport {
  ...
  private static void debug(String message, Object... args) {
    if (DEBUG || golo.lang.Runtime.debugMode()) {
  ...
  static {
    try {
      ...
      FALLBACK = lookup.findStatic(FunctionCallSupport.class, "fallback",
          methodType(Object.class, FunctionCallSite.class, Object[].class));
      SAM_FILTER = lookup.findStatic(FunctionCallSupport.class, "samFilter",
          methodType(Object.class, Class.class, Object.class));
      FUNCTIONAL_INTERFACE_FILTER = lookup.findStatic(FunctionCallSupport.class, "functionalInterfaceFilter",
  ...
  public static Object asFunctionalInterface(Lookup caller, Class<?> type, MethodHandle handle) throws Throwable {
    for (Method method : type.getMethods()) {
      if (!method.isDefault() && !isStatic(method.getModifiers())) {
        ...
        CallSite callSite = LambdaMetafactory.metafactory(caller, method.getName(), methodType(type), lambdaType,
            handle, lambdaType);
  ...
  public static CallSite bootstrap(Lookup caller, String name, MethodType type, Object... bsmArgs)
    ...
    FunctionCallSite callSite = new FunctionCallSite(caller, name.replaceAll("#", "\\."), type, constant,
        argumentNames);
  ...
  private static boolean isVarargsWithNames(Method method, Class<?>[] types, Object[] args, String[] argumentNames) {
    return method.isVarArgs() && (TypeMatching.isLastArgumentAnArray(types.length, args) || argumentNames.length > 0);
  }

  public static MethodHandle reorderArguments(Method method, MethodHandle handle, String[] argumentNames) {
    return NamedArgumentsHelper.reorderArguments(method.getName(), getParameterNames(method), handle, argumentNames, 0,
        0);
  }
    ...
    if (types != null) {
      for (int i = 0; i < types.length; i++) {
        if (TypeMatching.isSAM(types[i])) {
          ...
          handle = MethodHandles.filterArguments(handle, startIndex + i,
  ...
  private static AccessibleObject findClassWithConstructorFromImports(Class<?> callerClass, String classname,
      Object[] args) {
    String[] imports = Module.imports(callerClass);
    for (String imported : imports) {
      AccessibleObject result = findClassWithConstructor(callerClass, mergeImportAndCall(imported, classname), args);
      if (result != null) {
        return result;
      }
    }
  ...
  private static AccessibleObject findClassWithStaticMethodOrFieldFromImports(Class<?> callerClass, String functionName,
      Object[] args) {
    AccessibleObject result = null;
    if (functionName.contains(".")) {
      result = findClassWithStaticMethodOrField(callerClass,
          mergeImportAndCall(callerClass.getCanonicalName(), functionName), args);
      if (result != null) {
        return result;
      }
    }
    ...
    for (String importedClassName : imports) {
      result = findClassWithStaticMethodOrField(callerClass, mergeImportAndCall(importedClassName, functionName), args);
      if (result != null) {
        return result;
      }
    }
  ...
  private static AccessibleObject findClassWithStaticMethodOrField(Class<?> callerClass, String functionName,
      Object[] args) {
  ...
  private static AccessibleObject findStaticMethodOrField(Class<?> caller, Class<?> klass, String name,
      Object[] arguments) {
    ...
    Optional<Method> meth = Extractors.getMethods(klass)
        .filter(m -> methodMatches(caller, name, arguments, m, m.isVarArgs())).map(m -> checkDeprecation(caller, m))
        .findFirst();
    if (meth.isPresent()) {
      debug("method found");
      return meth.get();
    ...
    if (arguments.length == 0) {
      Optional<Field> f = Extractors.getFields(klass).filter(o -> fieldMatches(name, o))
          .map(o -> checkDeprecation(caller, o)).findFirst();
      return f.orElse(null);
    }
    return null;
  }

  private static boolean methodMatches(Class<?> caller, String name, Object[] arguments, Method method,
      boolean varargs) {
